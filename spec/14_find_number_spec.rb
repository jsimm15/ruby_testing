...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:rand_num) { double('random_number') }
      subject(:game) { described_class.new(0, 9, rand_num, 5)}

      it 'is game over' do
        allow(rand_num).to receive(:value).and_return(5)
    ...
    context 'when guess and random_number are not equal' do
      let(:rand_num) { double('random_number') }
      subject(:game) { described_class.new(0, 9, rand_num, 3) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'updates min to 5' do
        expect(low_guess_game.update_range).to eq(5)
      end
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'updates max to 8' do
        expect(high_guess_game.update_range).to eq(8)
      end
    end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:one_away_game) { described_class.new(5, 8, number_range, 7)}
      it 'updates min to the same value as max' do
        expect(one_away_game.update_range).to eq(one_away_game.max)
      end

      it 'does not update max' do
        expect(one_away_game.max).to eq(8)
      end
    end
  end
end
