...
describe Dog do
  ...
  subject(:fred) {described_class.new('Fred', 'Pit Bull', 'brown')}
  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'when Dog inherits methods from Pet class' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(fred).to_not be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(fred).to be_sleeping
  end
end
